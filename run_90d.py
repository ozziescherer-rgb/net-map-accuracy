"""90-day, 5-minute power flow -> true 15-min AMI averages (V15_90d, P15_90d)."""
import os, time
import numpy as np
import pandas as pd
...
for i, row in meta.iterrows():
    ...
    if str(row.cls).lower().startswith("residential"):
        ...
        nc = STEPS//3 + 2  # AR at 15-min scale
        ...
        if susc > 0.15:
            ...
            period = max(2, int(rng.uniform(10, 22)/STEP_MIN))
...
print(f"profiles: mean {P.mean():.2f} kW p99 {np.quantile(P,0.99):.1f}", flush=True)
...
for s in range(STEPS):
    ...
    if s % 4000 == 0: print(f"step {s}/{STEPS} {time.time()-t0:.0f}s", flush=True)

V15 = V.reshape(-1, 3, n).mean(1)   # 15-min true averages
P15 = P.reshape(-1, 3, n).mean(1)
np.save(f"{OUT}/V15_90d.npy", V15); np.save(f"{OUT}/P15_90d.npy", P15)
print(f"done {time.time()-t0:.0f}s nonconv={bad}; V15 {V15.shape} "
      f"mean {V15.mean():.4f} min {V15.min():.4f}")
